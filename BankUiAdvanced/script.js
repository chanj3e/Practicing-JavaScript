...
const account1 = {
  owner: 'Omar Atef',
  movements: [20000, 42055.23, -3060.5, 20000, -64.21, -1303.9, 76.97, 19000],
  interestRate: 1.2, // %
  pin: 1111,

  movementsDates: [
    '2024-04-18T21:31:17.178Z',
...
const startLogOutTimer = function () {
  const trick = function () {
    ...
    // when reaching 0 seconds log out
    if (time === 0) {
      clearInterval(timer);
...
const formatDate = function (date, locale) {
  const calcDaysTillNow = (day1, day2) =>
    Math.round(Math.abs(day1 - day2) / 1000 / 60 / 60 / 24);
  const days = calcDaysTillNow(new Date(), date);

  if (days === 0) return 'Today';
  if (days === 1) return 'Yasterday';
...
const calcDisplaySummary = function (acc) {
  ...
  labelSumIn.textContent = formateCur(deposits, acc.locale, acc.currency);
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    labelDate.textContent = Intl.DateTimeFormat(locale, options).format(date);
    // remember opacity With ppppppppp NOT b
    containerApp.style.opacity = 100;
...
btnTransfer.addEventListener('click', function (e) {
  ...
  if (
    ...
    recivedAcc.movementsDates.push(new Date().toISOString());
...
btnClose.addEventListener('click', function (e) {
  ...
  if (
    ...
    const currentIndex = accounts.findIndex(acc => currentAccount);
    // delete acc
